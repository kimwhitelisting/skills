...
- **이름**: `pptx`
- **설명**: .pptx 파일이 어떤 방식으로든(입력, 출력 또는 둘 다) 관련될 때마다 이 기술을 사용하십시오. 여기에는 슬라이드 데크, 프레젠테이션 데크 또는
          프레젠테이션 만들기, .pptx 파일에서 텍스트 읽기, 구문 분석 또는 추출(추출된 콘텐츠가 이메일이나 요약과 같은 다른 곳에서 사용되는 경우에도), 기존
          프레젠테이션 편집, 수정 또는 업데이트, 슬라이드 파일 결합 또는 분할, 템플릿, 레이아웃, 발표자 노트 또는 댓글 작업이 포함됩니다. 나중에 콘텐츠로 무엇을
          할 계획인지에 관계없이 "데크" "슬라이드" "프레젠테이션"을 언급하거나 .pptx 파일 이름을 참조하는 경우 이 기술을 사용하세요.
...
| 템플릿에서 편집 또는 만들기 | [editing.md](team_share_ko__skills__public__pptx__editing.md) 읽기 |
| 처음부터 만들기 | [pptxgenjs.md](team_share_ko__skills__public__pptx__pptxgenjs.md) 읽기 |
...
**자세한 내용은 [editing.md](team_share_ko__skills__public__pptx__editing.md)을 읽어보세요.**
...
**자세한 내용은 [pptxgenjs.md](team_share_ko__skills__public__pptx__pptxgenjs.md)을 읽어보세요.**
...
- **대담하고 내용에 맞는 색상 팔레트를 선택하세요**: 팔레트는 이 주제에 맞게 디자인되어야 합니다. 색상을 완전히 다른 표현으로 바꾸는 것이 여전히 "작동"한다면,
  구체적인 선택을 충분히 하지 않은 것입니다.

- **평등보다 우위**: 하나의 색상이 지배적이어야 하며(시각적 비중 60~70%), 1~2개의 보조 톤과 하나의 선명한 액센트가 있어야 합니다. 모든 색상에 동일한
  가중치를 부여하지 마십시오.

- **어두운/밝은 대비**: 제목 + 결론 슬라이드의 배경은 어둡고 내용의 배경은 밝습니다("샌드위치" 구조). 또는 프리미엄 느낌을 위해 전체적으로 어둡게 만듭니다.
- **시각적 모티브에 전념**: 둥근 이미지 프레임, 컬러 원으로 둘러싸인 아이콘, 두꺼운 단면 테두리 등 하나의 독특한 요소를 선택하고 반복합니다. 모든 슬라이드에서
  휴대하세요.
...
- **텍스트 상자 패딩을 잊지 마세요** — 텍스트 가장자리에 맞춰 선이나 모양을 정렬할 때 텍스트 상자에 `margin: 0`을 설정하거나 패딩을 고려하여 모양을
  오프셋하세요.
...
첫 번째 렌더링은 거의 정확하지 않습니다. 확인 단계가 아닌 버그 찾기로 QA에 접근하세요. 첫 번째 검사에서 문제가 전혀 발견되지 않았다면 충분히 자세히 살펴보지 않은
것입니다.
...
**⚠️ 하위 에이전트를 사용하세요** — 2~3개 슬라이드에도 가능합니다. 당신은 코드를 쳐다보고 거기에 있는 것이 아니라 기대하는 것을 보게 될 것입니다. 서브 에이전트는
신선한 눈을 가지고 있습니다.
